...
PROMPT_TEMPLATES = {
    ...
    "code_like": {
        "system": (
            "Please use the following Jinja syntax to process chaotic input text and output a standardized claim with one sentence:\n"
    ...
    if all(col in df.columns for col in ["post", "normalized claim"]):
        ...
        for index, row in first_rows.iterrows():
            ...
            if text:
                ...
                avg_score = all_score / (index - 1)
                print(f"{index - 1}\tMETEOR Score: {score:.4f}\tAVG METEOR Score: {avg_score:.4f}")
                    # 直接追加写入
                with open(output_file, 'a', encoding='utf-8') as f:
                    f.write(f"{index - 1}\t{avg_score:.4f}\t{score:.4f}\t{normal_claim}\t{text}\n")
            else:
                print("摘要生成失败")
